/**
 * match-maker-ui2.js — V2 Game UI for Nexus Arcade (SVG Edition)
 ...
 * Uses SVG icon sprites from /assets/svg/master-atlas.svg instead of emoji.
...
import { createIcon, injectAtlas }                     from './assets/js/ui-icons.js';
...
/** Per-element display metadata (SVG edition — no emoji field) */
const ELEMENT_DISPLAY = {
  radiant: { cls: 'gem-radiant', label: 'Radiant' },
  tide:    { cls: 'gem-tide',    label: 'Tide'    },
  verdant: { cls: 'gem-verdant', label: 'Verdant' },
  forge:   { cls: 'gem-forge',   label: 'Forge'   },
  aether:  { cls: 'gem-aether',  label: 'Aether'  },
  umbra:   { cls: 'gem-umbra',   label: 'Umbra'   },
  void:    { cls: 'gem-void',    label: 'Void'    },
};
...
function showNarrative(beat) {
  ...
  void dom.narrative.offsetWidth;
...
function renderBoard() {
  ...
  for (let r = 0; r < GRID_SIZE; r++) {
    for (let c = 0; c < GRID_SIZE; c++) {
      const gem  = grid[r][c];
      const info = (gem && ELEMENT_DISPLAY[gem.kind]) || { cls: '', label: '?' };
      const idx  = r * GRID_SIZE + c;
      ...
      // SVG icon (replaces emoji span in SVG edition)
      if (gem) {
        const icon = createIcon(gem.kind);
 ...
 */
export async function initMatchMakerV2(db, user) {
  // Inject SVG atlas for sprite rendering
  await injectAtlas();
  ...
  if (dom.canvas) {
    ...
    // Wire resize observer once so canvas stays in sync with board dimensions.
    if (!_resizeObserver && typeof ResizeObserver !== 'undefined') {
      const target = dom.board || dom.canvas;
      _resizeObserver = new ResizeObserver(() => { particles?.resize(); });
